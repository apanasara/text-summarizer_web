...
class SummaryTool{
	...
	/*Gets the sentences from the entire passage*/
	void extractSentenceFromContext(String arg){
		int sentenceStart=0;
        try{
	        	for (int i = 0; i<arg.length(); i++){
	        		...
	        		if(arg.charAt(i)== '.'){
	        			String sntnc = arg.substring(sentenceStart, i).trim();
	        			sentences.add(new Sentence(noOfSentences,sntnc,sntnc.length(),noOfParagraphs));
	    	        	noOfSentences++;
	    	        	sentenceStart=i+1;
	        		}
	...
	String getStats(){
		return "<b>number of words in Context</b> : " + getWordCount(sentences) + "<br/>" +
				"<b>number of words in Summary</b> : " + getWordCount(contentSummary) + "<br/>" +
				"<b>Commpression</b> : " + Math.round(10000*getWordCount(contentSummary)/ getWordCount(sentences))/100+ "% <br/>";
	}
	
	/* modified*/
	String getSummary(){
		String smry="<b>SUMMARY:</b>";
		for(Sentence sentence : contentSummary){
			smry = smry + "<br/>" + sentence.value;
		}
		return smry;
	}
}